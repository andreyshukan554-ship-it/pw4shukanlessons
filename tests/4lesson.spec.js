...
test.describe('Регистрация', () => {
    ...
    test('Пользователь зарегистрироваться с навигацией через клавиатуру', async ({page,}) => {
        ...
        // await fillRegistrationForm(page, user); далее в самой функции заполнения формы прописываем значения
        await expect(page.getByText(user.name)).toBeVisible();
        await expect(page.getByRole('navigation')).toContainText(user.name);
    });
});
*/
//const = {email, password, username} = user; данный способ позволяет быстро разобрать на переменные обьект