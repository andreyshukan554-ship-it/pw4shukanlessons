...
const URL = 'file:\\\D:\\my\\pw4\\demo.html';
...
test('поиск элемента по id', async ({ page }) => {
    ...
    await page.locator('#password').fill('supper');
...
test('поиск элемента по атрибуту', async ({ page }) => {
    ...
    await page.locator('[id="password"]').fill('supper');
    await page.locator('[id="btn"]').click();

    await expect(page.locator('[id="welcome-txt"]')).toContainText(
        'Привет');
...
//следующий способ не рекомендуется использовать
...
//  Будет работать по тексту но не плейсхолдеру  await page.locator('.el-input__inner').filter({hasText: 'Email'}).click();
    await page.locator('.el-input__wrapper').filter({has: page.locator('#email')}).click();
    await page.locator('.el-input__inner').first().fill('d@ya.ru');
    //last

    await page.locator('.el-input__inner').nth(1).click();
    await page.locator('.el-input__inner').nth(1).fill('supper');
    await page.locator('[id="btn"]').click();
